...
int main(int argc, char **argv) {
	...
	{ 
		...
		{ 
			...
			if (supernode <= 0) {
				po::error e("supernode must be greater than zero!");
				throw e;
			}
	...
	while(true) {
		...
		g.first.modifyByAddingNodes(5,.2);
		//g.first.modifyByAddingNodes(5,0.25);
		std::cout << "creating artificial node " << g.first.getNumberOfNodes()-1 << std::endl;
		...
		EmbeddingSpaceDominant<VertexInducedEmbedding> es(psize, supernode, &g.first, config);
		es.run_rw();	
		if(!ok) break;
	}
