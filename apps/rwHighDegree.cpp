...
int main(int argc, char **argv) {
	...
	{ 
		...
		desc.add_options() 
			...
			("degree,d", po::value<int>()->required(), "Degree threshold..") 
		...
		{ 
			...
			if (dthreshold<0) {
				po::error e("Degree threshold must be greatter than 0!");
				throw e;
			}
			if (supernode <= 0) {
	...
	while(true) {
		...
		if (thread > 1) { 
		      ...
		      es.setDegreeThreshold(dthreshold);
		      es.run_rw();	
		}
		else {
			EmbeddingSpaceHighDegree<VertexInducedEmbedding> es(psize, supernode, &g.first, config);
		        es.setAggOutput(output);
			es.setDegreeThreshold(dthreshold);
			es.run_rw();	
		}
